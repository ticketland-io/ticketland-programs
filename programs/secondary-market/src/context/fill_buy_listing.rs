...
pub struct FillBuyListing<'info> {
  ...
  pub buy_listing: Box<Account<'info, BuyListing>>,
  ...
  #[account(
    mut,
    constraint = ticket_buyer.key() == buy_listing.ticket_owner @ ErrorCode::WrongTicketSeller
  )]
  ...
  #[account(
    ...
    constraint = buy_listing.ticket_owner_purchase_token_ata == ticket_owner_purchase_token_ata.key() @ ErrorCode::WrongTicketOwnerPurchaseTokenAta
  )]
  ...
  pub ticket_owner_ata: Box<Account<'info, TokenAccount>>,
